...
```mermaid
flowchart TB
    subgraph DecodePath["Decode（1トークン / 1 forward）"]
        direction TB
        D1["活性化 X: 1×d（1行）"]
        D2["× 重み W: d×d'"]
        D3["→ 出力 Y: 1×d'"]
        D4["GEMV 的・Memory-bound 寄り"]
        D1 --> D2 --> D3 --> D4
    end
    subgraph PrefillPath["Prefill（Sトークン / 1 forward）"]
        direction TB
        P1["活性化 X: S×d（S行まとめ）"]
        P2["× 重み W: d×d'（1回読み）"]
        P3["→ 出力 Y: S×d'"]
        P4["GEMM・Compute-bound 寄り"]
        P1 --> P2 --> P3 --> P4
    end
```
...
Transformer の線形変換は、隠れ状態を行列 \(X \in \mathbb{R}^{S \times d}\)（\(S\) トークン × 隠れ次元 \(d\)）、重み \(W \in \mathbb{R}^{d \times d'}\) として \(Y = X W\) と書けます。形状は \((S \times d) \cdot (d \times d') \rightarrow (S \times d')\) で、これは **GEMM（General Matrix Multiply）** です。

Decode で1トークンだけ進めるときは \(X \in \mathbb{R}^{1 \times d}\) となり、**行列×ベクトル（GEMV）** に相当します（実装では \(M=1\) の細い GEMM として呼ばれることもあります）。

```mermaid
flowchart LR
    subgraph GEMM["Prefill: Y = X W（GEMM）"]
        direction LR
        Xb["X<br/>S×d<br/>S行の活性化"]
        Wb["W<br/>d×d'<br/>重み"]
        Yb["Y<br/>S×d'<br/>S行の出力"]
        Xb -->|"×"| Yb
        Wb -->|"×"| Yb
    end
```
...
```mermaid
flowchart LR
    subgraph GEMV["Decode: y = x W（GEMV 的）"]
        direction LR
        xb["x<br/>1×d<br/>1行だけ"]
        Wv["W<br/>d×d'<br/>同じ重み"]
        yv["y<br/>1×d'<br/>1行だけ"]
...
- **Decode（1トークン）**: 演算量はおおよそ \(2 d d'\) FLOPs。重み \(W\) を読んで **1行分だけ** 使う → 演算強度は \(O(1)\) に近く **Memory-bound** になりやすい
- **Prefill（\(S\) トークン）**: 演算量はおおよそ \(2 S d d'\) FLOPs。重み \(W\) を **1回読んで \(S\) 行すべてに使い回す** → 演算強度は \(S\) に比例して上がり **Compute-bound** 寄りになる

```mermaid
flowchart TB
    subgraph HBM["GPU HBM（重み W を保持）"]
        Wmem["重み W (d×d')"]
    end
    subgraph DecodeRead["Decode: 層あたり約1回読み → 1行だけ使用"]
        Dr1["forward #1"] --> Dr2["forward #2"] --> Dr3["…"] --> DrS["forward #S"]
        Wmem -.->|"毎ステップ読み直し"| Dr1
        Wmem -.->|"毎ステップ読み直し"| Dr2
        Wmem -.->|"毎ステップ読み直し"| DrS
    end
    subgraph PrefillRead["Prefill: 層あたり約1回読み → S行まとめて使用"]
        Pr1["1回の forward"]
        Wmem -->|"1回読み"| Pr1
        Pr1 --> UseS["S行ぶん FLOPs に再利用"]
    end
```

```mermaid
flowchart LR
    subgraph Roof["roofline の直感（線形層）"]
        direction TB
        R1["演算強度が低い<br/>（Decode / GEMV）"]
        R2["← メモリ帯域が上限<br/>Memory-bound"]
        R3["演算強度が高い<br/>（Prefill / GEMM, Sが大）"]
        R4["← 演算ユニットが上限<br/>Compute-bound 寄り"]
        R1 --> R2
        R3 --> R4
    end
```
...
Prefill（プロンプト長 \(S\)、因果マスクあり）と Decode（新トークン1つ、過去長 \(T\)）では、Attention 内の行列の **太さ** が異なります。

```mermaid
flowchart TB
    subgraph PrefillAttn["Prefill Attention"]
        direction TB
        PA1["Q, K, V 投影: 各 (S×d) → GEMM"]
        PA2["Q K^T: (S×d)(d×S) → (S×S) の大きな GEMM"]
        PA3["因果マスク適用"]
        PA4["softmax(QK^T) V: 再び GEMM"]
        PA1 --> PA2 --> PA3 --> PA4
    end
```
...
```mermaid
flowchart TB
    subgraph DecodeAttn["Decode Attention（1ステップ）"]
        direction TB
        DA1["新しい q: (1×d)"]
        DA2["KV cache の K: (T×d)"]
        DA3["q K^T: (1×d)(d×T) → (1×T) 細い積"]
        DA4["softmax → V 合成（帯域寄り）"]
        DA1 --> DA3
        DA2 --> DA3
        DA3 --> DA4
    end
...
```mermaid
flowchart LR
    subgraph Shapes["Attention スコア行列の形状イメージ"]
        direction TB
        subgraph Pshape["Prefill: Q K^T"]
            PS["S×S の正方形ブロック<br/>Tensor Core 向き"]
        end
        subgraph Dshape["Decode: q K^T"]
            DS["1×T の細い行ベクトル<br/>GEMV 的"]
        end
    end
```

- Prefill: \(Q, K, V\) の投影は GEMM。\(Q K^\top\) は \((S \times S)\) の **大きな GEMM**（マスクはその後）
- Decode: \(q K^\top\) は **細い行列×横長行列**（GEMV 的）。各ステップ \(O(T)\) だが **その1ステップは帯域寄り**

Prefill でも因果 Attention の総仕事量は \(O(S^2)\) ですが、**\(S \times S\) サイズの塊として Tensor Core に載せられる** 点が Decode の \(1 \times T\) とは異なります。

#### FFN は位置独立で並列が素直

Attention は因果制約（位置 \(i\) は \(1 \ldots i\) までしか参照しない）がありますが、**FFN は各位置で同じ重みを独立適用** します。

```mermaid
flowchart TB
    subgraph FFN["Prefill の FFN: 位置はバッチ次元"]
        direction LR
        Xff["X (S×d)<br/>t₁,t₂,…,tₛ の行"]
        W1["W₁"]
        W2["W₂"]
        Out["出力 (S×d)"]
        Xff -->|"全行まとめて GEMM"| W1
        W1 -->|"活性化 f"| W2
        W2 --> Out
        Out -.-> Note1["注: 各行は同じ W1,W2 を共有<br/>位置間の依存なし"]
    end
```

Prefill では \(\mathrm{FFN}(X) = f(X W_1) W_2\) の \(X\) が \((S \times d)\) なので、**位置方向は完全にバッチ次元** として GEMM に乗ります。
...
```mermaid
gantt
    title 同じプロンプト長 S の処理イメージ（1層・概念）
    dateFormat X
    axisFormat %s
    section Decode型（S回）
    forward t₁ :a1, 0, 1
    ...
    forward tₛ :a5, 4, 5
    section Prefill（1回）
    forward t₁…tₛ まとめ :b1, 0, 5
```

```mermaid
flowchart TB
    subgraph Bad["仮想: S回 × Decode型（非推奨）"]
        B1["GEMV × S回"]
        B2["重み HBM 読み ≈ S回/層"]
        B3["カーネル起動 ≈ S×層数"]
        B1 --> B2 --> B3
    end
    subgraph Good["実際: 1回 × Prefill"]
        G1["GEMM × 1回"]
        G2["重み HBM 読み ≈ 1回/層"]
        G3["カーネル起動 ≈ 層数"]
        G1 --> G2 --> G3
...
長いプロンプトをチャンク長 \(C\) で複数 forward に分けても、**各チャンク内では \((C \times d)\) の GEMM** です。

```mermaid
flowchart LR
    subgraph Full["1回の Prefill（理想）"]
        F1["X: S×d"] --> F2["1 forward<br/>GEMM 最大"]
    end
    subgraph Chunked["Chunked Prefill"]
        C1["chunk1: C×d"] --> CF1["forward #1"]
        C2["chunk2: C×d"] --> CF2["forward #2"]
        C3["chunk3: C×d"] --> CF3["forward #3"]
        CF1 --> CF2 --> CF3
    end
    subgraph Decode1["Decode（対比）"]
        D1["X: 1×d"] --> D2["毎トークン forward"]
    end
```

- \(C\) が小さい → GEMM の \(M\) 次元が小さく Tensor Core 効率・演算強度が落ちる（後述の **Chunked Prefill**）
- \(C\) が十分大きい（512〜8192 など）→ 依然として Decode の \(M=1\) より GEMM 寄り
- **「少数回」= スケジューリング上の分割** であり、各回の中身は依然として行列×行列
